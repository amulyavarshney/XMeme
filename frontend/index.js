...
function getContent(object) {
    ...
    height: 500px;
  ">
    <img src=${object.url} style="align: center; height: 320px; width:320px" onerror="this.onerror=null; this.src='images/invalid_url.jpg'" class="rounded float-start" alt="Image not found">
    <div class="card-body">
      <p class="card-text"><h4>${object.caption}</h4></p>
      <h3 class="card-title">Posted by : ${object.name}</h3>
      <button class="btn btn-primary" type="submit" id=${object.id} onclick="editMeme(${object.id})">Edit Post</button>
    </div>
  </div>
  </div>`
...
function getMemeList() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (this.readyState === 4 && this.status === 200) {
            ...
            document.getElementById("display-container").innerHTML = this.status + this.statusText + "<h1>Server is down. <span>Refresh the page!</span><h1>";
        }
    }
